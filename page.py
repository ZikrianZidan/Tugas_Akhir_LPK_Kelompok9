...
if (selected == 'About') :
    ...
    st.header('PPT pengenalan materi serta cara penggunaan aplikasi')
    
    with open(r'C:\Users\Zikrian\Downloads\PPT apk web.pdf', 'rb') as f:
        pdf_bytes = f.read()
        
    b64_pdf = base64.b64encode(pdf_bytes).decode('utf-8')
    st.markdown(f'<a href="data:application/pdf;base64,{b64_pdf}" download="PPT Aplikasi Web.pdf">Download PDF file</a>', unsafe_allow_html=True)
    
    st.caption(':red[*file dan aplikasi ini dibuat oleh kelompok 9 kelas 1H/Ankim Dual-System sebagai tugas akhir mata kuliah Logika dan Pemrograman Komputer]')
